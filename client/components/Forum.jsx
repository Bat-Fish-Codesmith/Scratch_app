...
const Forum = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      console.log(response, 'this is response in fetchMessages')
      const data = await response.json();
      setMessages(data);
    } catch (error) {
  ...
  const postMessage = async () => {
    try {
      console.log(newMessage, 'this is new message in post')
      const response = await fetch('http://localhost:3000/messages', {
        method: 'POST',
        // mode: 'no-cors',
        headers: {
  ...
  return (
    <div className="message-container">
      ...
      <div>
        {messages.map((message) => (
          <div key={message.message_id} className={"posts"}>
            {editingMessageId === message.message_id ? (
              <div>
                <input
                  ...
                  onChange={(e) => setNewMessage(e.target.value)}
                />
                <button onClick={() => updateMessage(message.message_id)}>Save</button>
                <button onClick={() => setEditingMessageId(null)}>Cancel</button>
              </div>
            ) : (
              <div>
                <p>{message.content}</p>
                <button onClick={() => deleteMessage(message.message_id)} className="delete-btn">Delete</button>
                <button onClick={() => updateMessage(message.message_id)} className="update-btn">Edit</button>
              </div>
            )}
          </div>
        ))}
      ...
      <form onSubmit={postMessage}>
        <input
          ...
          className='text-bar'
        />
        <button onClick={postMessage} className="post-msg">Post Message</button>
      </form>
    </div>
  );
};
